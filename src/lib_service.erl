...
start_template([{App,Vsn}|T],LoaderVm,Acc)->
    Result=case rpc:call(LoaderVm,loader,load_appl,[App,node()],10000) of
	       {error,Reason}->
		   {error,[Reason,App,Vsn]};
	       ok->
		   case rpc:call(LoaderVm,loader,start_appl,[App,node()],10000) of
		        {error,Reason}->
			   {error,[Reason,App,Vsn]};
		       ok->
			   case rpc:call(node(),App,ping,[],2000) of
			       pong->
				   {ok,[App,Vsn]};
			       {badrpc,Reason}->
